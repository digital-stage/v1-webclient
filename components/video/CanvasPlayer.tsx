    ...
    componentDidUpdate(prevProps: Readonly<Props>, prevState: Readonly<States>, snapshot?: any): void {
        if (prevProps.consumers !== this.props.consumers
            ...
            if (this.state.size) {
                ...
                const currentAnimationFrames: AnimationFrame[] = this.state.animationFrames.filter((animationFrame: AnimationFrame) => videoTracks.find(track => track.id === animationFrame.id));

                if (numRows > 0) {
                    const numColsMax = Math.ceil(videoTracks.length / numRows);

                    const elementWidth = Math.round(this.state.size.width / numColsMax);
                    const elementHeight = Math.round(this.state.size.height / numRows);
                    ...
                    for (let i = 0; i < videoTracks.length; i++) {
                        ...
                        const animationFrame = currentAnimationFrames.find((animationFrame: AnimationFrame) => animationFrame.id === track.id);
                        if (!animationFrame) {
    ...
    render() {
        return (
            <div className={this.props.className} ref={this.wrapperRef}>
                <Canvas ref={this.canvasRef} width={this.state.size && this.state.size.width}
