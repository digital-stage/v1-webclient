        ...
        const socket = io(process.env.NEXT_PUBLIC_API_URL, {
          ...
          query: {
            ...
            device: JSON.stringify({
              ...
              inputAudioDeviceId: devices.inputAudioDevices.find(
                (d) => d.id === "label"
              )
                ? "default"
                : devices.inputAudioDevices.length > 0
                ? devices.inputAudioDevices[0].id
                : undefined,
              inputVideoDeviceId:
                devices.inputVideoDevices.length === 1
                  ? devices.inputVideoDevices[0].id
                  : "default",
              outputAudioDeviceId: devices.outputAudioDevices.find(
                (d) => d.id === "label"
              )
                ? "default"
                : devices.outputAudioDevices.length > 0
                ? devices.outputAudioDevices[0].id
                : undefined,
            } as Device),
          },
        });
