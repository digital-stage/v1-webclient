...
import {
    AudioConsumer,
    AudioProducer, CustomAudioProducer, CustomGroup,
    CustomOvTrack,
    CustomStageMember,
    Device,
    Group,
    OvTrack,
    Stage,
    StageMember, User, VideoConsumer,
    VideoProducer
} from "../../model";

const user = new schema.Entity<User>('users');

const device = new schema.Entity<Device>('devices', {
    user: user
}, {idAttribute: '_id'});

const audioConsumer = new schema.Entity<AudioConsumer>('audioConsumers', {}, {idAttribute: '_id'});

const videoConsumer = new schema.Entity<VideoConsumer>('audioConsumers', {}, {idAttribute: '_id'});

const audioProducer = new schema.Entity<AudioProducer>('audioProducers', {}, {idAttribute: '_id'});

const customAudioProducer = new schema.Entity<CustomAudioProducer>('customAudioProducers', {
    audioProducer: audioProducer
}, {idAttribute: '_id'});

const videoProducer = new schema.Entity<VideoProducer>('videoProducers', {}, {idAttribute: '_id'});

const ovTrack = new schema.Entity<OvTrack>('ovTracks', {}, {idAttribute: '_id'});

const customOvTrack = new schema.Entity<CustomOvTrack>('customOvTracks', {
    ovTrack: ovTrack
}, {idAttribute: '_id'});
...
const customGroup = new schema.Entity<CustomGroup>('customGroups', {}, {idAttribute: '_id'});

const group = new schema.Entity<Group>('groups', {
    stageMembers: [stageMember],
    customGroup: customGroup
}, {idAttribute: '_id'});

const stage = new schema.Entity<Stage>('stages', {
        admins: [user],
        groups: [group],
        stageMembers: [stageMember],
