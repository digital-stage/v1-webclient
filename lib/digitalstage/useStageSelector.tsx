...
import {AudioConsumer, Group, OvTrack, Stage, StageMember, VideoConsumer} from "./useStageContext/model";
...
export function useCurrentStage(): Stage {
    ...
    const stage = useSelector<NormalizedState, Stage>(state => stageId ? state.stages.byId[stageId] : undefined, shallowEqual);
    return stage;
}

export function useCurrentGroup(): Group {
    const groupId = useSelector<NormalizedState, string>(state => state.groupId, shallowEqual);
    return useSelector<NormalizedState, Group>(state => groupId ? state.groups.byId[groupId] : undefined, shallowEqual);
}
...
export function useGroupsByStage(stageId: string): Group[] {
    return useSelector<NormalizedState, Group[]>(state => stageId ? state.groups.byStage[stageId] ? state.groups.byStage[stageId].map(id => state.groups.byId[id]) : [] : [], shallowEqual);
}

export function useIsStageAdmin(): boolean {
    return useSelector<NormalizedState, boolean>(state => state.stageId ? state.stages.byId[state.stageId].isAdmin : false);
}

export function useStageMembers(): ExtendedStageMember[] {
    const stageMembers = useSelector<NormalizedState, StageMembers>(state => state.stageMembers, shallowEqual);
    ...
    const audioConsumers = useSelector<NormalizedState, AudioConsumers>(state => state.audioConsumers, shallowEqual);
    ...
    return stageMembers.allIds
        ...
        .map(stageMember => {
            return {
                ...
                videoConsumers: videoConsumers.byStageMember[stageMember._id] ? videoConsumers.byStageMember[stageMember._id].map(id => videoConsumers.byId[id]) : [],
                audioConsumers: audioConsumers.byStageMember[stageMember._id] ? audioConsumers.byStageMember[stageMember._id].map(id => audioConsumers.byId[id]) : [],
                ovTracks: ovTracks.byStageMember[stageMember._id] ? ovTracks.byStageMember[stageMember._id].map(id => ovTracks.byId[id]) : [],
            }
        })
}
