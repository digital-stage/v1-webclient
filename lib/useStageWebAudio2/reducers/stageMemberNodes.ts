...
export function stageMemberNodesReducer(prev: GainAudioNodeCollection = {
    byId: {}
}, action: StageMemberNodesActions): GainAudioNodeCollection {
    switch (action.type) {
        case 'CHANGED':
            ...
            action.stageMembers.forEach(item => {
                ...
                if (targetNodes) {
                    ...
                    const customItem: ThreeDimensionAudioProperties = action.customStageMembers.byStageMember[item._id]
                    ...
                    if (!state.byId[item._id]) {
                        ...
                        state.byId[item._id] = updateGainAudioNode(targetNodes, state.byId[item._id], item, customItem);
