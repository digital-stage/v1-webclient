...
export function audioProducerNodesReducer(
  prev: AudioProducerNodesCollection = {
    byId: {},
  },
  action: AudioProducerNodesActions
): AudioProducerNodesCollection {
  switch (action.type) {
    case 'CHANGED':
      ...
      action.audioProducers.forEach((audioProducer) => {
        ...
        if (targetNodes) {
          ...
          const customAudioProducer = action.customAudioProducers.byAudioProducer[audioProducer._id]
            ? action.customAudioProducers.byId[
                action.customAudioProducers.byAudioProducer[audioProducer._id]
              ]
            : undefined;
          const audioConsumer = action.audioConsumers.byProducer[audioProducer._id]
            ? action.audioConsumers.byId[action.audioConsumers.byProducer[audioProducer._id]]
            : undefined;
          ...
          const customStageMember = action.customStageMembers.byStageMember[stageMember._id]
            ? action.customStageMembers.byId[
                action.customStageMembers.byStageMember[stageMember._id]
              ]
            : undefined;
